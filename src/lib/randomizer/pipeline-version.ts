 ...
 *   • Order or content changes to HARDCODED_CLASS_SKILLS, HARDCODED_CLASS_SKILLS_ORDER,
 *     SKILL_CLASS_EXCLUSIONS, RESTRICT2_COPACED, CLASS_SPECIFIC_ANIMS
 *     (src/lib/randomizer/skill-placer.ts)
 *   • Order changes to CLASS_DEFS or CHARCLASS_TO_CODE
 *     (src/lib/randomizer/config.ts)
 *   • Changes to the sort key in placeSkills() or the isPinnedToOriginalClass() predicate
